...
: P-value results for Hypothesis 1 {#tbl-h1_pvalue .striped .hover tbl-colwidths="[50,50]"}  

</div>

We can reasonably conclude from this that in 10 of our 14 focus medications, there is a statistically significant difference in mean prices and reject the null hypothesis for these medications. (See @tbl-h1_pvalue above.) To dive deeper, our team constructed density plots to compare the direction of this difference.  As observed in @fig-h1, where we evaluate two of these medications, Semaglutide and Dulaglutide.  We can see that the direction shifts at different price ranks and the shift depends on the medication.
...
Drilling down again, with these two medications in focus, actual values are compared below in @tbl-h1_means.
...
: Mean of Mean Price and Standard Deviation for Dulaglutide and Semaglutide betwen two race groups {#tbl-h1_means .striped .hover tbl-colwidths="[25,25,25,25]"}  
...
: P-value results for Hypothesis 2 {#tbl-h2_pvalue .striped .hover tbl-colwidths="[50,50]"}  

</div>

Again, we find that there are statistically significant outcomes for many of the selected medications.  Twelve of the fourteen medications yield p-values below 0.05 suggesting that the null hypothesis be rejected. (See @tbl-h2_pvalue above.)
...
: Tukey HSD test comparing Semaglutide prices amongst four poverty levels {#tbl-p2_tukey .striped .hover tbl-colwidths="[40, 15, 15, 15, 15]"}  
...
: Mean and standard deviation of prices of Semaglutide by four poverty buckets {#tbl-p2-means .striped .hover tbl-colwidths="[30, 14, 14, 14, 14]"} 
...
p1 <- ggplot(train_metrics, aes(x = Model, y = Drug, fill = Rsquared)) +
  ...
  scale_fill_viridis_c(option = "magma", direction = -1, name = bquote(R^2), limits = Rsq_limits) + 
  labs(title = bquote("Training Data " ~ R^2), x = "Model", y = NULL) +
  theme_minimal(base_size = 12) +
...
p2 <- ggplot(test_metrics, aes(x = Model, y = Drug, fill = Rsquared)) +
  ...
  scale_color_identity() +  # use colors as-is from the column
  scale_fill_viridis_c(option = "magma", direction = -1, name = bquote(R^2), limits = Rsq_limits) +
  labs(title = bquote("Test Data " ~ R^2), x = "Model", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
...
ggplot(all_varimp_clean, aes(x = Drug_Model, y = Variable, fill = NormalizedImportance)) +
  ...
  geom_text(aes(label = round(NormalizedImportance, 1), color = TextColor), 
            size = 3, 
            angle = 0) +
  scale_fill_viridis_c(option = "magma", direction = -1, name = "Normalized\nImportance") +
  scale_color_identity() +  # Use colors as-is from the column
...
ggplot(df, aes(x = PercentChange, y = MedianPrice,
  ...
  geom_text_repel(aes(label = Drug), size = 3, max.overlaps = 10, box.padding = 0.6, point.padding = 0.6) +
  labs(
    title = "Effect of Additional Available Medicare Plans in a County on Drug Price",
    x = "% Change in Price per 10 Additional Medicare Plans",
