...
#| fig-cap: "Variance in price amongst the top drugs (by absolute spending) across the US"
#| warning: false
...
box_plot_table <- box_plot_table %>%
  mutate(drug_name_clean = factor(drug_name_clean, levels = rev(custom_order)))

# Create the box plot
ggplot(box_plot_table, aes(x = drug_name_clean, y = UNIT_COST)) +
  geom_boxplot() + 
  labs(title = "Variance in price amongst the top drugs (by absolute spending descending order)",
       x = "",
