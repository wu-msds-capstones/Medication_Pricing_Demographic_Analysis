...
To better understand the implications of this finding, we introduced an affordability measure by dividing the mean price by the median income of each county. This revealed that although prices were lower in higher-poverty areas, the cost represented a larger proportion of income—highlighting a disparity in affordability. This relationship is clearly illustrated in the following table and Figure @fig-h2_2.
...
We employed both t-tests and Tukey HSD tests to analyze a range of medications across various demographic subsets, tailoring each test to our hypotheses, exploring the potential for demographic price impacts. These analyses produced a diverse set of outputs, many of which indicated statistical significance. Upon closer inspection, we observed marginal differences in the average mean prices across county groups, as well as in our affordability metric and other summary statistics.

While some differences were statistically significant, they often lacked practical relevance. It's important to note that statistical significance merely suggests that a difference is unlikely to be due to random chance—it does not necessarily imply that the difference is meaningful in a real-world context. As we delved deeper into the results, it became clear that although our tests were identifying differences, these findings did not always translate into actionable insights.

To better understand the implications of these results, we shifted our focus toward evaluating how population sizes compare to the number of available plans in each area. This perspective allows us to assess whether plan availability aligns with population needs, offering a more grounded view of affordability and access across different regions.
